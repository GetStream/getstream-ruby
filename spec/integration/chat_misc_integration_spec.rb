...
RSpec.describe 'Chat Misc Integration', type: :integration do
  ...
  describe 'CreateListDeleteDevice' do
    ...
    it 'creates a firebase device, lists it, deletes it, and verifies gone' do
      ...
      # List devices
      list_resp = @client.common.list_devices(@user_1)
      devices = list_resp.devices || []
      found = devices.any? do |d|

        h = d.is_a?(Hash) ? d : d.to_h
      ...
      # Delete device
      @client.common.delete_device(device_id, @user_1)

      # Verify deleted
      list_resp_2 = @client.common.list_devices(@user_1)
      devices_2 = list_resp_2.devices || []
      still_found = devices_2.any? do |d|
      ...
      if e.message.include?('push provider') || e.message.include?('no push')
        skip('Push providers not configured for this app')
      end
      raise
  ...
  describe 'CreateListDeleteBlocklist' do
    ...
    it 'creates a custom blocklist, lists it, verifies found, and deletes it' do
      ...
      # Verify update
      get_resp_2 = @client.common.get_block_list(blocklist_name)
      bl_h_2 = get_resp_2.blocklist.to_h
      expect(bl_h_2['words'].length).to eq(4)
  ...
  describe 'CreateListDeleteCommand' do
    ...
    it 'creates a custom command, lists it, verifies found, and deletes it' do
      ...
      # Verify update
      get_resp_2 = @client.make_request(:get, "/api/v2/chat/commands/#{cmd_name}")
      expect(get_resp_2.description).to eq('Updated test command')
  ...
  describe 'CreateUpdateDeleteChannelType' do
    ...
    it 'creates a channel type, updates settings, verifies, and deletes' do
      ...
      # 3000, so stay within that ceiling to avoid silent truncation.
      ...
      # Update channel type — raise to 3000 (plan maximum) to verify the
      ...
      10.times do
        ...
        break if max_message_length == 3000

        sleep(0.5)

      end
      expect(max_message_length).to eq(3000)
  ...
  describe 'QueryBannedUsers' do
    ...
    it 'bans a user in channel, queries banned users, and verifies' do
      ...
      # Query banned users
      payload = JSON.generate({ filter_conditions: { 'channel_cid' => { '$eq' => cid } } })
      ...
      bans_2 = resp_2.bans || []
      expect(bans_2.length).to eq(0), 'Bans should be empty after unban'
  ...
  describe 'MuteUnmuteUser' do
    ...
    it 'mutes user, verifies via query, and unmutes' do
      ...
      q_resp = @client.common.query_users(JSON.generate({
                                                          filter_conditions: { 'id' => { '$eq' => @user_1 } },
                                                        }))
  ...
  describe 'ExportChannels' do
    ...
    it 'exports channel messages and polls task until completed' do
      ...
      send_test_message('messaging', channel_id, @user_1, "Message for export test #{SecureRandom.hex(4)}")
  ...
  describe 'Threads' do
    ...
    it 'creates parent + replies, queries threads, and verifies' do
      ...
      parent_id = send_test_message('messaging', channel_id, @user_1, 'Thread parent message')
  ...
  describe 'GetUnreadCounts' do
    ...
    it 'sends message and gets unread counts for user' do
      ...
      _type, channel_id, _resp = create_test_channel_with_members(@user_1, [@user_1, @user_2])
      send_test_message('messaging', channel_id, @user_1, "Unread test #{SecureRandom.hex(4)}")
  ...
  describe 'GetUnreadCountsBatch' do
    ...
    it 'gets unread counts for multiple users' do
      ...
      _type, channel_id, _resp = create_test_channel_with_members(@user_1, [@user_1, @user_2])
      send_test_message('messaging', channel_id, @user_1, "Batch unread test #{SecureRandom.hex(4)}")
      ...
      expect(counts_h.key?(@user_1)).to be(true)
      expect(counts_h.key?(@user_2)).to be(true)
  ...
  describe 'Reminders' do
    ...
    it 'creates a reminder, lists it, updates it, and deletes it' do
      ...
      _type, channel_id, _resp = create_test_channel_with_members(@user_1, [@user_1])
      msg_id = send_test_message('messaging', channel_id, @user_1, "Reminder test #{SecureRandom.hex(4)}")
  ...
  describe 'SendUserCustomEvent' do
    ...
    it 'sends a custom event to a user' do
      ...
      resp = @client.make_request(:post, "/api/v2/chat/users/#{@user_1}/event", body: {
                                    event: {
  ...
  describe 'QueryTeamUsageStats' do
    ...
    it 'queries team usage stats' do
      ...
      if e.message.include?('Token signature') ||
         e.message.include?('not available') ||
         e.message.include?('not found') ||
         e.message.include?('Not Found')
        skip('QueryTeamUsageStats not available on this app')
      end
      raise
  ...
  describe 'ChannelBatchUpdate' do
    ...
    it 'batch updates multiple channels at once' do
      ...
      _type_1, ch_id_1, _resp_1 = create_test_channel(@user_1)
      _type_2, ch_id_2, _resp_2 = create_test_channel(@user_1)

      # Batch update: set a custom field on both channels
      cids = ["messaging:#{ch_id_1}", "messaging:#{ch_id_2}"]
      ...
      if e.message.include?('not available') ||
         e.message.include?('Not Found') ||
         e.message.include?('unknown') ||
         e.message.include?('not found')
        skip('Channel batch update not available')
      end
      raise
