...
RSpec.describe 'Feed Integration Tests', type: :integration do
  ...
  let(:test_user_id) { nil }
  let(:test_user_id2) { nil }
  ...
  describe 'Activity Operations' do
    it 'creates, retrieves, and updates activities' do
      ...
      @test_user_id = 'test-user-ruby-sdk1'
      puts @test_user_id
      test_helper.create_test_feed(feed_group_id, feed_id, @test_user_id)
      
      # snippet-start: CreateActivity
      # Create activity
      activity_id = test_helper.create_test_activity(feed_group_id, feed_id, @test_user_id, 'Test activity for CRUD operations')
    ...
    it 'queries activities with filters' do
      ...
      3.times do |i|
        test_helper.create_test_activity(feed_group_id, feed_id, @test_user_id, "Query test activity #{i + 1}")
      end
  ...
  describe 'Reaction Operations' do
    it 'adds and queries reactions' do
      ...
      test_helper.create_test_feed(feed_group_id, feed_id, @test_user_id)
      activity_id = test_helper.create_test_activity(feed_group_id, feed_id, @test_user_id, 'Activity for reaction test')
      ...
      # Delete reaction
      delete_response = client.feeds.delete_activity_reaction(activity_id, 'like', @test_user_id)
  ...
  describe 'Comment Operations' do
    it 'adds, queries, and updates comments' do
      ...
      test_helper.create_test_feed(feed_group_id, feed_id, @test_user_id)
      activity_id = test_helper.create_test_activity(feed_group_id, feed_id, @test_user_id, 'Activity for comment test')
  ...
  describe 'Bookmark Operations' do
    it 'adds, queries, and updates bookmarks' do
      ...
      test_helper.create_test_feed(feed_group_id, feed_id, @test_user_id)
      activity_id = test_helper.create_test_activity(feed_group_id, feed_id, @test_user_id, 'Activity for bookmark test')
      ...
      begin
        ...
        # Delete bookmark
        delete_response = client.feeds.delete_bookmark(activity_id, folder_id, @test_user_id)
        expect(delete_response).to be_a(GetStreamRuby::StreamResponse)
        puts "✅ Deleted bookmark successfully"
        
      rescue => e
        puts "⚠️ Bookmark operations skipped: #{e.message}"
        skip "Bookmark operations not supported: #{e.message}"
      end
    end
  end
  ...
  describe 'Follow Operations' do
    it 'creates and manages follows' do
      ...
      begin
        # Follow user
        follow_request = {
          source: "#{feed_group_id}:#{@test_user_id}",
        ...
        # Unfollow user
        unfollow_response = client.feeds.unfollow("#{feed_group_id}:#{@test_user_id}", "#{feed_group_id}:#{@test_user_id2}")
        ...
        skip "Follow operations not supported: #{e.message}"
      end
    end
  end
  ...
  describe 'Pin Operations' do
    it 'pins and unpins activities' do
      ...
      test_helper.create_test_feed(feed_group_id, feed_id, @test_user_id)
      activity_id = test_helper.create_test_activity(feed_group_id, feed_id, @test_user_id, 'Activity for pin test')
      ...
      # Unpin activity
      unpin_response = client.feeds.unpin_activity(feed_group_id, feed_id, activity_id, @test_user_id)
  ...
  describe 'Poll Operations' do
    it 'creates and votes on polls' do
      ...
      begin
        ...
        puts "⚠️ Poll operations skipped: #{e.message}"
        skip "Poll operations not supported: #{e.message}"
      end
    end
  end
  ...
  describe 'Feed View CRUD Operations' do
    it 'performs feed view CRUD operations' do
      ...
      begin
        ...
        # Get or create feed view
        get_or_create_request = {
          aggregation: { type: 'default' }
        }
        
        get_or_create_response = client.feeds.get_or_create_feed_view(feed_view_id, get_or_create_request)
        expect(get_or_create_response).to be_a(GetStreamRuby::StreamResponse)
        puts "✅ Got existing feed view successfully"
        
      rescue => e
        puts "⚠️ Feed View CRUD operations skipped: #{e.message}"
        skip "Feed View CRUD operations not supported: #{e.message}"
      end
    end
  end
