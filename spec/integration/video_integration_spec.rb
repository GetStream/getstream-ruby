...
RSpec.describe 'Video Integration', type: :integration do
  ...
  describe 'CRUDCallTypeOperations' do
    ...
    it 'creates a call type with settings, updates, reads, and deletes' do
      ...
      3.times do |i|
        ...
        resp_2 = @client.make_request(:put, "/api/v2/video/calltypes/#{ct_name}", body: {
                                        settings: {
      ...
      expect(resp_2).not_to be_nil

      # Read call type (with retry)
      resp_3 = nil
      3.times do |i|

        resp_3 = @client.make_request(:get, "/api/v2/video/calltypes/#{ct_name}")
        break
      rescue GetStreamRuby::APIError
        raise if i == 2
      ...
      expect(resp_3.name).to eq(ct_name)
  ...
  describe 'BlockUnblockUserFromCalls' do
    ...
    it 'blocks a user from a call and unblocks' do
      ...
      expect(blocked_ids).to include(@user_2)
      ...
      5.times do
        ...
        sleep(1)
        resp_2 = get_call('default', call_id)
        call_h_2 = resp_2.to_h
        blocked_ids_2 = call_h_2.dig('call', 'blocked_user_ids') || []
        unless blocked_ids_2.include?(@user_2)
          unblocked = true
          break
        end
  ...
  describe 'SendCustomEvent' do
    ...
    it 'sends a custom event in a call' do
      ...
      resp = @client.make_request(
        ...
        body: { user_id: @user_1, custom: { bananas: 'good' } },
      )
      expect(resp).not_to be_nil
  ...
  describe 'CreateCallWithSessionTimer' do
    ...
    it 'creates a call with max_duration_seconds and updates it' do
      ...
      # Update to 7200
      resp_2 = update_call('default', call_id, {
                             settings_override: {
                               limits: { max_duration_seconds: 7200 },
                             },
                           })
      call_h_2 = resp_2.to_h
      max_dur_2 = call_h_2.dig('call', 'settings', 'limits', 'max_duration_seconds')
      expect(max_dur_2).to eq(7200)

      # Reset to 0
      resp_3 = update_call('default', call_id, {
                             settings_override: {
                               limits: { max_duration_seconds: 0 },
                             },
                           })
      call_h_3 = resp_3.to_h
      max_dur_3 = call_h_3.dig('call', 'settings', 'limits', 'max_duration_seconds')
      expect(max_dur_3).to eq(0)
  ...
  describe 'UserBlocking' do
    ...
    it 'blocks and unblocks a user at app level' do
      ...
      # Verify unblocked
      resp_2 = @client.common.get_blocked_users(alice)
      blocks_2 = resp_2.blocks || []
      blocked_ids = blocks_2.map do |b|
  ...
  describe 'CreateCallWithBackstageAndJoinAhead' do
    ...
    it 'creates a call with backstage and join_ahead_time_seconds' do
      ...
      call_h_2 = resp_2.to_h
      join_ahead_2 = call_h_2.dig('call', 'settings', 'backstage', 'join_ahead_time_seconds')
      expect(join_ahead_2).to eq(600)
      ...
      call_h_3 = resp_3.to_h
      join_ahead_3 = call_h_3.dig('call', 'settings', 'backstage', 'join_ahead_time_seconds')
      expect(join_ahead_3).to eq(0)
  ...
  describe 'EnableCallRecordingAndBackstageMode' do
    ...
    it 'updates call settings for recording and backstage' do
      ...
      # Enable recording
      resp_1 = update_call('default', call_id, {
                             settings_override: {
                               recording: { mode: 'available', audio_only: true },
                             },
                           })
      call_h_1 = resp_1.to_h
      expect(call_h_1.dig('call', 'settings', 'recording', 'mode')).to eq('available')
      ...
      call_h_2 = resp_2.to_h
      expect(call_h_2.dig('call', 'settings', 'backstage', 'enabled')).to eq(true)
